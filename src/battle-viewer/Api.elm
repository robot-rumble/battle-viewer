...
getRobotCode : Context -> Bool -> RobotId -> Request String
getRobotCode context isDev robotId =
    let
        path =
            if isDev then
                context.paths.getDevCode

            else
                context.paths.getPublishedCode
    in
    Get ( path, [ String.fromInt (unwrapRobotId robotId) ], string )
